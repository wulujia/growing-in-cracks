...
做到什么程度？我了解到的一家大型内容平台，同时跑着超过 20 个安全模型：色情、低俗、暴恐、领导人、好坏二分类（兜底模型）、高危视频消重、OCR 图文、违禁品。审核流程是：用户发布后内容仅自己可见，先过安全模型矩阵，高风险进人工审核，低风险进质量模型判断推荐。

人工审核处理机器无法判断的内容。机器给出“疑似风险”的判断，人工做最终确认。人工审核结果反过来可以训练机器模型，形成闭环。

审核模式有两种。先审后发：内容过审后才可见，安全优先。先发后审：内容先上线，后台持续审核，发现问题再处理，效率优先。知识星球的全部内容都是先审后发。

有内容平台的做法是：爆火内容至少经过四级人工审核，每层审核关注不同维度并逐级严格。

### 风控引擎

规则和模型配合使用：规则处理已知的、确定的风险（比如关键词命中就拦截），模型捕捉未知的、模糊的风险（比如从行为模式判断异常）。AI 时代，还可以训练小模型专门做内容审核，减轻人工压力。
...
前面提到的"用户染色"，其实就是信用体系的雏形。把它做完整，就是给每个用户一个信用分：实名信息、历史行为、违规记录、社交关系，综合打分。

信用分高的用户，发内容直接放行。信用分低的，自动进人工审核队列。发生纠纷时，信用高者优先受理。黑产账号的特征（注册信息异常、社交关系单一、行为轨迹反常）也能通过信用分识别。

要注意的是，信用体系得给用户改过的机会，不能变成永久黑名单。规则也要透明——用户不知道为什么被降权，就会觉得不公平。

### 管理制度

内容安全不只是管用户发的东西。公司官方账号发的内容、员工个人社交媒体上的言论，都在管理范围内。几件事要做到位：
...
还有一件容易忽略的事：各主管部门发布的信息内容管理规定，其实是很实用的风控指引，值得认真读一遍。
