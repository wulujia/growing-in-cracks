...
2025 年，OpenClaw 的官方插件中心 ClawHub 被发现大量恶意 skill。有一个叫“X (Twitter) Trends”的插件，外观完全正常，但里面藏着 Base64 编码的后门命令。用户安装后，插件会悄悄下载并执行恶意代码。这不是个别现象，慢雾安全团队一共发现了大约 472 个恶意 skill。这就是 AI 时代的供应链攻击——你以为装了一个好用的工具，其实给自己的电脑开了一扇后门（图 36-1）。

![图 36-1 ClawHub 恶意 skill 示例](img/ai-01.png)
